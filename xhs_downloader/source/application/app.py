...
class XHS:
    ...
    def __normalize_login_redirect(self, url: str) -> str:
        if not url:
            return url
        parsed_url = urlparse(url)
        if parsed_url.path != "/login":
            return url
        query_map = parse_qs(parsed_url.query)
        redirect_values = query_map.get("redirectPath")
        if not redirect_values:
            return url
        redirect_url = unquote(redirect_values[0])
        redirect_parsed = urlparse(redirect_url)
        note_identifier = redirect_parsed.path.rstrip("/").split("/")[-1]
        token_values = parse_qs(redirect_parsed.query).get("xsec_token")
        if not note_identifier or not token_values:
            return url
        token = token_values[0]
        if not token:
            return url
        return f"https://www.xiaohongshu.com/explore/{note_identifier}?xsec_token={token}"
    ...
    async def extract_links(self, url: str, log) -> list:
        ...
        for i in url.split():
            ...
            if not i:
                continue
            i = self.__normalize_login_redirect(i)
    ...
    def __generate_data_object(self, html: str) -> Namespace:
        data: dict[Any, Any] = self.convert.run(html)
